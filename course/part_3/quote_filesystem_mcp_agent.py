...
async def model_call(state: AgentState) -> AgentState:
    system_prompt = SystemMessage(
        content="В твоем распоряжении есть инструмменты для работы с файловой системой и инструмент для получения мотивационных цитат на русском языке."
    )
    ...
    result = await app.ainvoke(
        {
            "messages": [
                HumanMessage(
                    content="Найди мотивационную цитату на русском языке и сохрани ее в файл quote.txt со всей информацией, которую ты найдешь по автору цитаты."
                )
            ]
        }
    )
